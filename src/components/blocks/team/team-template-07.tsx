...
export function TeamTemplate07({ config, language }: BlockProps) {
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid md:grid-cols-2 gap-8">
          {c.items.map((member, i) => (
            <div
              key={i}
              className={`flex flex-col sm:flex-row gap-6 bg-gray-50 rounded-2xl overflow-hidden border border-gray-100 ${
            ...
            >
              ...
              <div className="sm:w-2/5 shrink-0">
                {member.photo ? (
                  ...
                  <div
                    className="w-full h-48 sm:h-full flex items-center justify-center text-white text-5xl font-bold"
                    style={{
              ...
              {/* Info - 60% */}
              <div className={`sm:w-3/5 p-6 flex flex-col justify-center ${isAr ? "text-right" : ""}`}>
                ...
                {member.socials && member.socials.length > 0 && (
                  <div className={`flex items-center gap-2 mt-4 ${isAr ? "justify-end" : ""}`}>
                    {member.socials.map((social, si) => (
                      social && (
                      <a
                        key={si}
                        href={social.url || "#"}
                        className="w-8 h-8 rounded-full bg-gray-200 flex items-center justify-center text-gray-500 hover:text-white transition-all duration-200"
                        onMouseEnter={(e) => {
