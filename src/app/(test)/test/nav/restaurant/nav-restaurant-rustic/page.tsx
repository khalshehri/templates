...
import { NavRestaurantRustic } from "./nav-restaurant-rustic";

export default function NavRestaurantRusticTestPage() {
    ...
    <div dir={language === "ar" ? "rtl" : "ltr"}>
      <div className="sticky top-0 z-50 bg-gray-950/90 backdrop-blur border-b border-gray-800 px-6 py-3 flex items-center justify-between">
        <div className="flex items-center gap-4">
          <Link href="/test/nav/restaurant" className="text-gray-400 hover:text-white text-sm">&larr; Back</Link>
          <span className="text-white font-semibold">Nav Restaurant Rustic</span>
        </div>
      ...
      <div className="min-h-screen" style={{ background: "#3d2e1f" }}>
        <NavRestaurantRustic language={language} />
      </div>
    </div>
  );
}
