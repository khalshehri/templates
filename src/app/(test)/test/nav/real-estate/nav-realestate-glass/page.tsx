...
import { NavRealEstateGlass } from "./nav-realestate-glass";

export default function NavRealEstateGlassTestPage() {
    ...
    <div dir={language === "ar" ? "rtl" : "ltr"}>
      <div className="sticky top-0 z-50 bg-gray-950/90 backdrop-blur border-b border-gray-800 px-6 py-3 flex items-center justify-between">
        <div className="flex items-center gap-4">
          <Link href="/test/nav/real-estate" className="text-gray-400 hover:text-white text-sm">← Back</Link>
          <span className="text-white font-semibold">Nav RealEstate Glass</span>
        </div>
      ...
      <div className="min-h-screen" style={{ background: "linear-gradient(135deg, #1a365d 0%, #2d3748 50%, #1a202c 100%)" }}>
        <NavRealEstateGlass language={language} />
      </div>
    </div>
  );
}
